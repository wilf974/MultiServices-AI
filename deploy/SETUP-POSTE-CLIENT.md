...
- Endpoint central : `https://mem.woutils.com` — lecture `/mcp` (bearer), écriture `/ingest` (mTLS + HMAC).
- **Prérequis** : l'IP publique du poste doit être ajoutée à l'allowlist du vhost `mem.woutils.com` (sinon 403).
- **Secrets** (NE JAMAIS committer / logger) :
  - `BEARER` (lecture) : dans `/etc/nginx/sites-enabled/mem.woutils.com` (ligne `map $http_authorization $mem_ok`).
...
```
claude mcp add --transport http multiservice-memory https://mem.woutils.com/mcp --header "Authorization: Bearer <BEARER>"
```
...
```bash
ssh -p 2299 adminvps@mem.woutils.com        # ou adminvps@192.168.1.210 depuis le LAN
...
HMAC=$(sudo cat /home/adminvps/mem-secrets/clients/$NOM/hmac.key)
# inscrire au registre (cree ou met a jour le JSON) :
sudo python3 -c 'import json,os,sys;p="/home/adminvps/mem-secrets/ingest-clients.json";r=json.load(open(p)) if os.path.exists(p) else {};r[sys.argv[1]]={"source":"project:"+sys.argv[1],"hmac_key":sys.argv[2]};json.dump(r,open(p,"w"),indent=2);print("registre ->",list(r))' "$NOM" "$HMAC"
# exposer les 3 fichiers pour scp (compte adminvps) :
sudo cp /home/adminvps/mem-secrets/clients/$NOM/client.crt /home/adminvps/mem-secrets/clients/$NOM/client.key /home/adminvps/mem-secrets/clients/$NOM/hmac.key /home/adminvps/
sudo chown adminvps:adminvps /home/adminvps/client.crt /home/adminvps/client.key /home/adminvps/hmac.key
```
Récupérer sur le poste (un `scp` par fichier ; PowerShell ne fait pas l'expansion `{a,b,c}`) :
```
scp -P 2299 adminvps@mem.woutils.com:/home/adminvps/client.crt <DEST>
scp -P 2299 adminvps@mem.woutils.com:/home/adminvps/client.key <DEST>
scp -P 2299 adminvps@mem.woutils.com:/home/adminvps/hmac.key  <DEST>
```
`<DEST>` = `C:\mem` (Windows) ou `~/mem` (Linux). Puis nettoyer le VPS :
```bash
rm /home/adminvps/client.crt /home/adminvps/client.key /home/adminvps/hmac.key
```
...
```
$env:MEM_INGEST_URL="https://mem.woutils.com/ingest"; $env:MEM_CLIENT_CERT="C:\mem\client.crt"; $env:MEM_CLIENT_KEY="C:\mem\client.key"; $env:MEM_HMAC_KEY=(Get-Content C:\mem\hmac.key -Raw).Trim(); memlog-http "<FAIT>" --kind <decision|correction|observation|validation|note|hypothesis> --session "<sujet-stable>"
```

### bash (Git Bash / Linux, certs dans `/c/mem` ou `~/mem`)
```
MEM_INGEST_URL="https://mem.woutils.com/ingest" MEM_CLIENT_CERT="/c/mem/client.crt" MEM_CLIENT_KEY="/c/mem/client.key" MEM_HMAC_KEY="$(tr -d '\r\n' < /c/mem/hmac.key)" memlog-http "<FAIT>" --kind decision --session "<sujet-stable>"
```
