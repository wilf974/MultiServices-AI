...
- Modify: `deploy/mem.woutils.com.nginx` (mTLS au server + `location /ingest`)
...
OUT="/home/adminvps/mem-secrets/clients/$CN"
mkdir -p "$CADIR" "$OUT"
...
echo "Entree registre a ajouter dans /home/adminvps/mem-secrets/ingest-clients.json :"
echo "  \"$CN\": { \"source\": \"$SRC\", \"hmac_key\": \"$HMAC\" }"
```

- [ ] **Step 4 : Modifier `deploy/mem.woutils.com.nginx`** — ajouter au bloc `server` (après les `ssl_*` existants) :
...
```bash
git add deploy/Dockerfile.ingest deploy/docker-run-ingest.sh deploy/gen-mtls.sh deploy/mem.woutils.com.nginx deploy/README.md
...
- Produces: `https://mem.woutils.com/ingest` opérationnel ; poste(s) capables d'écrire.
...
git archive --format=tar feature/ingest-distant | \
  ssh -p 2299 adminvps@192.168.1.210 'tar -x -C /home/adminvps/mem-mcp-src'
```

- [ ] **Step 2 : CA + cert client + clé HMAC** (root)

```bash
mkdir -p /home/adminvps/mem-secrets
bash /home/adminvps/mem-mcp-src/deploy/gen-mtls.sh bureau project:bureau
# creer/mettre a jour /home/adminvps/mem-secrets/ingest-clients.json avec l'entree imprimee
```
Expected: CA créée, `client.crt`/`client.key`/`hmac.key` sous `/home/adminvps/mem-secrets/clients/bureau/`.

- [ ] **Step 3 : Conteneur d'ingest** (adminvps)

```bash
ssh -p 2299 adminvps@192.168.1.210 'cd ~/mem-mcp-src && docker build -f deploy/Dockerfile.ingest -t mem-ingest . && bash deploy/docker-run-ingest.sh && docker ps --filter name=mem-ingest'
```
...
```bash
cd "/c/Users/Wilfred/Claude/Projects/MultiService IA"
python -m multiservice.projlog "Ingest distant EN SERVICE : POST /ingest mTLS sur mem.woutils.com, conteneur mem-ingest (journal RW, registre :ro hors .aethercore), provenance imposee par CN. Poste bureau ecrit dans la memoire centrale." --kind decision --source project:MultiService-IA --session ingest-distant
```
