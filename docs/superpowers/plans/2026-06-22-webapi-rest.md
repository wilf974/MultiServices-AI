...
**Files:**
- Create: `deploy/Dockerfile.webapi`, `deploy/docker-run-webapi.sh`, `deploy/gen-webapi-token.sh`, `deploy/api-mem.woutils.com.nginx`
- Modify: `deploy/README.md`

**Interfaces:**
- Consumes: console script `multiservice-webapi`, registre `token->source`.
- Produces: image `mem-api`, conteneur `127.0.0.1:8304`, vhost public `api-mem.woutils.com`.
...
echo "  $TOKEN"
echo "Entree a ajouter dans /home/adminvps/mem-secrets/webapi-tokens.json :"
echo "  \"$TOKEN\": { \"source\": \"$SRC\" }"
```

- [ ] **Step 4 : `deploy/api-mem.woutils.com.nginx`**
...
server {
    ...
    server_name api-mem.woutils.com;

    ssl_certificate     /etc/letsencrypt/live/api-mem.woutils.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api-mem.woutils.com/privkey.pem;
...
1. **Token** (root) : `bash deploy/gen-webapi-token.sh chatgpt project:chatgpt`
   puis ajouter l'entree imprimee dans `/home/adminvps/mem-secrets/webapi-tokens.json`.
2. **Conteneur** : `docker build -f deploy/Dockerfile.webapi -t mem-api . && bash deploy/docker-run-webapi.sh`
3. **DNS + TLS** : pointer `api-mem.woutils.com` vers le VPS ; `certbot` pour le cert ;
   appliquer `deploy/api-mem.woutils.com.nginx` ; `nginx -t && systemctl reload nginx`.
4. **Client web** : `Authorization: Bearer <token>` sur `https://api-mem.woutils.com`
...
```bash
git add deploy/Dockerfile.webapi deploy/docker-run-webapi.sh deploy/gen-webapi-token.sh deploy/api-mem.woutils.com.nginx deploy/README.md
...
Suivre `deploy/README.md` § « API REST web » : DNS `api-mem.woutils.com`, `certbot`, `gen-webapi-token.sh`, registre, `docker build`/`run`, vhost nginx, `nginx -t && systemctl reload nginx`. Verif bout-en-bout : `curl -H "Authorization: Bearer <token>" https://api-mem.woutils.com/health` -> 200 ; `/recall` sans token -> 401.
