...
- **Allowlist IP** : `82.64.87.227` (maison/public) et `193.252.56.212` (bureau).
...
- Create: `deploy/mem.woutils.com.nginx`
...
docker run -d --name mem-mcp --restart unless-stopped \
  ...
  -v /home/adminvps/.aethercore:/data:ro \
...
`deploy/mem.woutils.com.nginx` (remplacer `__MEM_READ_TOKEN__` et les chemins de certificat au déploiement) :
...
server {
    ...
    allow 82.64.87.227;     # maison / IP publique
    allow 193.252.56.212;   # bureau
    deny all;
...
```bash
git add deploy/
git commit -m "feat(deploy): image Docker (journal :ro) + site nginx + guide mem.woutils.com"
```
...
- Produces: endpoint `https://mem.woutils.com/mcp` opérationnel ; clients connectés.

- [ ] **Step 1 : DNS** — créer `mem.woutils.com` A → `82.64.87.227` (TTL 300). *(utilisateur)*
...
- [ ] **Step 3 : Construire + lancer le conteneur** (VM, adminvps — docker accessible sans root)

```bash
ssh -p 2299 adminvps@192.168.1.210 'cd ~/multiservice && docker build -f deploy/Dockerfile.mem -t mem-mcp . && bash deploy/docker-run-mem.sh && docker ps --filter name=mem-mcp'
```
...
Générer `MEM_READ_TOKEN` (`openssl rand -hex 32`), instancier le site, `certbot --nginx -d mem.woutils.com`, `nginx -t && systemctl reload nginx`.
Expected: `nginx -t` = `syntax is ok` / `test is successful`.

- [ ] **Step 5 : Smoke tests** (depuis une IP allowlistée)

```bash
curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer <TOKEN>" https://mem.woutils.com/mcp   # 200/handshake
curl -s -o /dev/null -w "%{http_code}\n" https://mem.woutils.com/mcp                                       # 401
```
...
```bash
claude mcp add --transport http multiservice-memory https://mem.woutils.com/mcp --header "Authorization: Bearer <TOKEN>"
claude mcp list
```
