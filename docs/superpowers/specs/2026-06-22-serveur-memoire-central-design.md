...
poste client (IP allowlistée, n'importe quel réseau)
   │  claude mcp add --transport http https://mem.woutils.com/mcp
   │  Authorization: Bearer <MEM_READ_TOKEN>
   ▼
nginx :443 (mem.woutils.com)  ── TLS (certbot)
   │  allow 82.64.87.227; allow 193.252.56.212; deny all;
...
/home/adminvps/.aethercore/journal-llm.jsonl   = source de vérité centrale
```
...
**DNS** : `mem.woutils.com` → A `82.64.87.227`, TTL 300 (IP publique du site ; la VM 192.168.1.210
...
- `deploy/docker-run-mem.sh` :
  `docker run -d --name mem-mcp --restart unless-stopped -p 127.0.0.1:8302:8302 \
   -v /home/adminvps/.aethercore:/data:ro -e MULTISERVICE_JOURNAL=/data/journal-llm.jsonl mem-mcp`
- `deploy/mem.woutils.com.nginx` : site nginx (voir §6).
...
server {
  listen 443 ssl;
  server_name mem.woutils.com;        # ssl_certificate via certbot
...
- **HTTPS only** (80 → 443).
- **Allowlist IP** : `82.64.87.227`, `193.252.56.212` (extensible).
...
- **Smoke post-déploiement** (checklist manuelle) :
  - `curl -H "Authorization: Bearer <TOKEN>" https://mem.woutils.com/mcp` (depuis IP allowlistée) → 200/handshake ;
...
1. DNS `mem.woutils.com` A → 82.64.87.227 (TTL 300). *(fait par l'utilisateur)*
2. `docker build -f deploy/Dockerfile.mem -t mem-mcp .` puis `deploy/docker-run-mem.sh`.
3. Site nginx `deploy/mem.woutils.com.nginx` → `/etc/nginx/sites-enabled/`, certbot, `nginx -t && reload`.
...
| Hairpin NAT (poste maison) | egress vu = 82.64.87.227, déjà allowlisté |
```
