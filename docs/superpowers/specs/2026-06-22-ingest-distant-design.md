...
**Objectifs**
- Endpoint `POST /ingest` sur `mem.example.com`, **mTLS obligatoire**, qui valide et append un
...
poste (cert client mTLS + cle HMAC + memlog-http)
  --TLS + mTLS--> nginx :8443 (mem.example.com)  [allowlist IP]
...
- Ajout `location /ingest` au vhost `deploy/mem.example.com.nginx` + directives mTLS au server.
