...
- Exposition : **sous-domaine public `api-mem.woutils.com`**, token-only, rate-limité, **sans allowlist IP**.
...
- Registre **`/home/adminvps/mem-secrets/webapi-tokens.json`** (hors `~/.aethercore`), monté **`:ro`** :
  ```json
...
- `deploy/api-mem.woutils.com.nginx` : vhost public (TLS Let's Encrypt), `limit_req`, **sans** allowlist IP,
  `proxy_pass http://127.0.0.1:8304`. (Le `mem.woutils.com` IP-restreint reste inchangé.)
