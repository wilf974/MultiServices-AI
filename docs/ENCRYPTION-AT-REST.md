...
> défaut (`decrypt_or_tombstone` opt-in, migration forward-only). `read_events(path, keyring=...)` **branché** (opt-in, décryptage/tombstone).
> Reste : CLI `encrypt_migrate`, `LogKeyStore` à l'échelle. Design issu d'un aller-retour
