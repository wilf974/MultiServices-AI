...
def test_source_project_isole_du_chat(tmp_path):
    ...
    jp = tmp_path / "journal.jsonl"
    log(jp, "decision", "Licence Apache-2.0 pour le projet", session_id="licence", now=T0)  # source project:local
    # un event facon chat eve (source user:local) qui parle aussi de licence/Apache
    eve = AetherEvent(type=EventType.PROMPT, title="prompt", description="config Apache reverse proxy",
                      source="user:local", observed_at=T0,
